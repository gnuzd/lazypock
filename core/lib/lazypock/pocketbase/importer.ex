defmodule Lazypock.PocketBase.Importer do
  ...
  defp import_collection_schema(pb_coll, id_to_name, opts) do
    name = pb_coll["name"]
    type = if pb_coll["type"] == "auth", do: "auth", else: "base"
  ...
  defp map_field(f, id_to_name) do
    ...
    lazy_type =
      case pb_type do
        ...
        "bool" -> "bool"
        "date" -> "date"
        "select" -> if multi?(f), do: "multi_select", else: "select"
        "file" -> if multi?(f), do: "multi_file", else: "file"
        "relation" -> "relation"
        "json" -> "json"
        "password" -> "password"
        other ->
  ...
  defp import_records(lazy_coll, pb_coll, records, storage_dir, db_path, full_id_map) do
    ...
    Enum.reduce(records, {0, 0, 0, %{}, []}, fn record,
                                               {count, files, auths, id_map, warnings} ->
      old_id = record["id"]
      ...
      case insert_record(lazy_coll, record, new_id) do
        :ok ->
          ...
          {count + 1, files + files_copied, auths + auths_added,
           Map.put(id_map, old_id, new_id), warnings}
  ...
  defp coerce_for_column(%{type: t}, value) when t in ["date", "datetime"], do: parse_pb_time(value)
  defp coerce_for_column(%{type: "bool"}, value), do: value == true or value == 1
  defp coerce_for_column(%{type: "json"}, value) when is_map(value) or is_list(value), do: Jason.encode!(value)
