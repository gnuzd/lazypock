defmodule LazypockWeb.SettingsController do
  ...
  defp do_update(conn, params) do
    data = Map.drop(params, ["_method", "_csrf_token"])

    upsert_settings(data)
  ...
  defp do_import(conn, params) do
    ...
    Enum.reduce_while(collections, {:ok, [], []}, fn coll_data, {:ok, imported_acc, errors_acc} ->
      name = coll_data["name"]
      type = coll_data["type"] || "base"
      schema = coll_data["schema"] || []
      _rules = coll_data["rules"] || %{}
      records = coll_data["records"] || []

      case Lazypock.Schema.DDL.create_collection(name, type: type, fields: schema) do
        {:ok, _collection} ->
          inserted =
    ...
    |> case do
      {:ok, imported_list, errors_list} ->
        json(conn, %{imported: Enum.reverse(imported_list), errors: Enum.reverse(errors_list)})

      _ ->
        conn |> put_status(500) |> json(%{error: "Import failed"})
    end
  end
end
