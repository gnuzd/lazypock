<script lang="ts">
	...
	async function runSql(data: { query: string }) {
		sqlError = '';
		sqlResults = null;
		try {
			const res = (await client.http.post('/sql/query', { sql: data.query })) as Record<
				string,
				unknown
			> | null;
			if (res?.error) {
...
>
	...
	<div class="mt-2 flex items-center gap-2">
		<Button class="btn-primary" loading={sqlForm.submitting} type="submit">Run Query</Button>
		<button
			type="button"
