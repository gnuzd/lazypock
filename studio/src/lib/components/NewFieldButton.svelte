...
<Dropdown bind:show={open} class="my-2">
	...
	<div class="flex min-w-[300px] flex-row flex-wrap gap-0 p-1.5">
		{#each addableFieldTypes as type (type)}
			<button
				type="button"
				class="my-0.5 flex w-1/4 cursor-pointer items-center gap-2 rounded-field border-none bg-transparent p-2 text-left text-sm text-base-content transition-[background] duration-(--animation-speed) outline-none hover:bg-base-200"
				onmousedown={(e) => {
					e.preventDefault();
					addField(type);
				}}
			>
				...
				{:else if type === 'bool'}
					<svg
						...
						stroke="currentColor"
						stroke-width="2"
						><rect x="2" y="6" width="20" height="12" rx="6" /><circle cx="8" cy="12" r="2" /></svg
					>
				{:else if type === 'email'}
					<svg
						width="14"
				...
				{:else if type === 'json'}
					<svg
						...
						stroke="currentColor"
						stroke-width="2"
						><path d="M14.5 3.5L17 6l-2.5 2.5" /><path d="M9.5 3.5L7 6l2.5 2.5" /><path
							d="M12 20l4-10"
						/><path d="M4 20h16" /></svg
					>
				{:else if type === 'file'}
